...
namespace UltraPool
{
    public class ObjectPoolManager
    {
        ...
        /// <summary>
        /// Returns object(type) from pool if not object create pool.
        /// </summary>
        /// <param name="item"></param>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public T GetObjectAuto<T>(T item) where T : MonoBehaviour
        {
            if (_pools.ContainsKey(item.name) == false)
            {
                AddObjectPool(
                    ()=> BaseFactor(item),
            ...
            return ((ObjectPool<T>)_pools[item.name]).GetObject();
        }
        ...
        public T GetObjectAuto<T>(GameObject prefab) where T : MonoBehaviour
        {
            if (prefab == null)
                Debug.LogError("No prefab");

            if (_pools == null)
                Debug.LogError("No pools");
            
            if (_pools != null && !_pools.ContainsKey(prefab.name))
            {
                AddObjectPool(
                    () => BaseFactorObject<T>(prefab),
                    (x) => x.gameObject.SetActive(true),
                    (x) => x.gameObject.SetActive(false),
                    prefab.name,
                    1, true);
        ...
        private T BaseFactorObject<T>(GameObject prefab) where T : MonoBehaviour
        {
            GameObject instance = Object.Instantiate(prefab);
            instance.name = prefab.name; // Set the name to match the prefab name and pool name
        ...
        public void ReturnObject<T>(T o) where T : MonoBehaviour
        {
            var prefab = o.gameObject;
            foreach (var pool in _pools)
            {
                if (pool.Value is ObjectPool<T> objectPool && prefab.name.StartsWith(pool.Key))
                {
                    objectPool.ReturnObject(o);
                    return;
                }
            }
            Debug.LogWarning($"No pool found to return object of type {typeof(T)}.");
        }
